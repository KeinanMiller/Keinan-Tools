...
New-Item -path c:\Installs\HealthCheck\$ComputerName -type Directory -ErrorAction Ignore
$filename = 'c:\Installs\HealthCheck\$ComputerName'+ $ComputerName + '-' + $UserName + '.csv'
   ...
   if ($Connection -eq "True"){
      ...
      $ComputerBios = Get-WmiObject win32_bios -ComputerName $ComputerName | Select-Object SMBIOSBIOSVersion,Manufacturer,Name,SerialNumber,Version
      ...
      $ComputerInfoBiosSerial = $ComputerBios.SerialNumber
      $ComputerInfoBiosVersion = $ComputerBios.Version
      ...
      $ComputerInfo | Add-Member -MemberType NoteProperty -Name “BiosSerial” -Value “$ComputerInfoBiosSerial”-Force
...
$ComputerDisks = ""
#}
$Inventory | Export-Csv -Path $filename